...
class ClientA {
    ...
    async run() {
        ...
        const data = await this._runQuery(aClient, queries.schemaQuery);
        ...
        // filter out aggregate and array fields
        responseObject.fields = responseObject.fields.filter(field => (!field.description || !field.description.includes("relationship") && !field.description.includes("array")));

        const q = this._buildQuery(schema, responseObject);
        if (!q) throw `can't build query for ${rqQueryName}`;
        this._log(`query: ${q}`);

        const response = await this._runQuery(aClient, q);
    ...
    async _runQuery(gqClient, q) {
        this._log("running query");
        const r = await gqClient.query({ 'query': gql(q) });
        return r.data;
    };
